    ...
    public class Auction {
        ...
        private static final String PASSED_BID_SQL = "SELECT bid_id, coalesce(bidder_id, 0) AS BIDDER_ID, BID_PRICE\n" +
                "FROM BID\n" +
                "JOIN ITEM I on BID.ITEM_ID = I.ITEM_ID\n" +
                "WHERE BID_CLOSING_DATE < NOW() AND I.STATUS = 'Y';";
        private static final String UPDATE_ITEM_STATUS_SQL = "UPDATE ITEM\n" +
                "SET STATUS = 'N'\n" +
                "WHERE ITEM_ID = (SELECT ITEM_ID FROM BID WHERE BID_ID = ?);";
        private static final String BID_PURCHASE_SQL = "WITH BID_INFO AS (\n" +
        ...
        private static void bidProcess() {
            /**
             * 1. check all the items (Now() > bid_end_time)
             * 2. if no one bidded on the item, just turn the status to N
             * 3. if anyone has bidded on the item turn the status to N and do purchase process
             */
            try {
                // 1
                PreparedStatement pstmt1 = conn.prepareStatement(PASSED_BID_SQL);
                ResultSet rs1 = pstmt1.executeQuery();
                List<Integer> bidIdsWithBidder = new ArrayList<>();
                List<Integer> bidIdsWithoutBidder = new ArrayList<>();
                while (rs1.next()) {
                    int bidId = rs1.getInt("bidder_id");
                    if (bidId != 0) {
                        bidIdsWithBidder.add(bidId);
                    } else {
                        bidIdsWithoutBidder.add(bidId);
                    }
                }
                // 2
                for (Integer b : bidIdsWithoutBidder) {
                    PreparedStatement pstmt2 = conn.prepareStatement(UPDATE_ITEM_STATUS_SQL);
                    pstmt2.setInt(1, b);
                    pstmt2.executeUpdate();
                    pstmt2.close();
                }
                // 3
                for (Integer b : bidIdsWithBidder) {
                    PreparedStatement pstmt2 = conn.prepareStatement(UPDATE_ITEM_STATUS_SQL);
                    pstmt2.setInt(1, b);
                    pstmt2.executeUpdate();
                    pstmt2.close();
                    PreparedStatement pstmt3 = conn.prepareStatement(BID_PURCHASE_SQL);
                    pstmt3.setInt(1, b);
                ...
                System.out.println("Some problem occurred while processing bid info. Please try again.");
                return;
            }
        }
        ...
        private static boolean AdminMenu() {
            ...
            try {
                ...
                try {
                    PreparedStatement pstmt = conn.prepareStatement(ADMIN_LOGIN_SQL);
        ...
        public static void main(String[] args) {
            ...
            try{
                conn = DriverManager.getConnection("jdbc:postgresql://localhost/"+args[0], args[1], args[2]);
